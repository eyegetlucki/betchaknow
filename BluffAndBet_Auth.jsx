...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .input-wrap input:focus {
    ...
    box-shadow:0 0 0 3px ${C.accent4}22;
  }
  .input-wrap input.error {
    border-color:${C.accent1};
    box-shadow:0 0 0 3px ${C.accent1}22;
  }
  .input-icon {
  ...
  .oauth-btn:hover { border-color:${C.border2}; background:#13101f; transform:translateY(-1px); }
  ...
  return (
    <div style={{ textAlign:"center", animation:"fadeUp 0.5s ease" }}>
      ...
      <h1 style={{
        ...
        <span style={{ color:C.accent1 }}>Bluff</span>
        <span style={{ color:C.text }}> & </span>
        <span style={{ color:C.accent2 }}>Bet</span>
      </h1>
      {big && (
      ...
      <AuthCard style={{ animation: shake ? "shake 0.4s ease" : "popIn 0.4s ease" }}>
        <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
          ...
          <button
            ...
            style={{
              background:`linear-gradient(135deg, ${C.accent4}, ${C.accent5})`,
              color:"#fff",
              boxShadow:`0 4px 24px ${C.accent4}44`,
            }}
          >
            {loading ? (
      ...
      <AuthCard style={{ animation: shake ? "shake 0.4s ease" : "popIn 0.4s ease" }}>
        <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
                ...
                <div style={{ display:"flex", flexWrap:"wrap", gap:"4px 16px" }}>
                  {checks.map((c,i) => (
                    <div key={i} className="check-item"
                      style={{ color: c.ok ? C.accent3 : C.muted2 }}>
                      <span>{c.ok ? "✓" : "○"}</span>
                      <span>{c.label}</span>
                    </div>
                  ))}
          ...
          <button className="primary-btn" disabled={loading} onClick={handleSubmit}
            style={{
              background:`linear-gradient(135deg, ${C.accent1}, ${C.accent5})`,
    ...
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center",
      ...
      <AuthCard style={{ textAlign:"center", padding:"48px 32px" }}>
        ...
        <p style={{ color:C.muted, fontSize:14, marginTop:8,
                    fontFamily:"'Plus Jakarta Sans',sans-serif", fontWeight:500 }}>
          Welcome to Bluff & Bet. Loading your account...
        </p>
      ...
      <AuthCard>
        <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
          ...
          <button className="primary-btn" disabled={loading || code.join("").length < 6}
            ...
            style={{
              background:`linear-gradient(135deg, ${C.accent3}, ${C.accent4})`,
          ...
          {/* Security note */}
          <div style={{
            background:"#0d0b1a", border:`1px solid ${C.border2}`,
            borderRadius:12, padding:"12px 16px",
            display:"flex", gap:10, alignItems:"flex-start",
          }}>
            <span style={{ fontSize:16, flexShrink:0 }}>🔐</span>
            <p style={{ color:C.muted, fontSize:12, fontFamily:"'Plus Jakarta Sans',sans-serif",
                        fontWeight:500, lineHeight:1.5, margin:0 }}>
              Your account uses industry-standard encryption. We never store plain text passwords or share your data with third parties.
            </p>
          </div>
...
export default function App() {
  const [screen, setScreen] = useState("login"); // login | signup | verify | forgot

  const handleSuccess = (dest) => {
    if (dest === "verify") setScreen("verify");
    if (dest === "game")   setScreen("game");
  };
  ...
  if (screen === "game") return (
    <div style={{
      ...
      <style>{css}</style>
      <Orbs />
      <div style={{ textAlign:"center", zIndex:1, animation:"popIn 0.5s ease" }}>
        <div style={{ fontSize:64, marginBottom:16, animation:"floatA 4s ease-in-out infinite" }}>🎯</div>
        <h2 style={{ fontFamily:"'Boogaloo',cursive", fontSize:40, color:C.text, marginBottom:8 }}>
          <span style={{color:C.accent1}}>Bluff</span> & <span style={{color:C.accent2}}>Bet</span>
        </h2>
        <p style={{ color:C.accent3, fontSize:16, fontWeight:700, marginBottom:24 }}>
          ✓ Successfully signed in!
        </p>
        <p style={{ color:C.muted, fontSize:14 }}>
          Next: Profile & Badge Showcase →
        </p>
        <div style={{ marginTop:24, display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap" }}>
          {["login","signup","forgot"].map(s => (
            <button key={s} onClick={() => setScreen(s)}
              style={{
                padding:"8px 18px", borderRadius:10, border:`1.5px solid ${C.border2}`,
                background:"transparent", color:C.muted, cursor:"pointer",
                fontFamily:"'Plus Jakarta Sans',sans-serif", fontWeight:700, fontSize:13,
              }}>
              View {s === "login" ? "Login" : s === "signup" ? "Sign Up" : "Forgot PW"} →
            </button>
          ))}
        </div>
      </div>
