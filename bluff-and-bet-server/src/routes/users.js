...
router.post("/me/avatar", requireAuth, asyncHandler(async (req, res) => {
  ...
  if (!imageBase64 || !contentType) throw createError(400, "Missing image data");
  if (!contentType.startsWith("image/")) throw createError(400, "File must be an image");

  const buffer = Buffer.from(imageBase64, "base64");
  if (buffer.length > 5 * 1024 * 1024) throw createError(400, "Image too large (max 5 MB)");
  ...
  if (uploadErr) throw createError(500, uploadErr.message);

  const { data: { publicUrl } } = supabaseAdmin.storage.from("avatars").getPublicUrl(path);
  const urlWithBust = `${publicUrl}?t=${Date.now()}`;

  await supabaseAdmin.from("profiles").update({ avatar_icon: urlWithBust }).eq("id", req.user.id);

  res.json({ avatarUrl: urlWithBust });
}));
