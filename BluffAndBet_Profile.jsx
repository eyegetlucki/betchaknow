...
const css = `
  ...
  *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
  ...
  .badge-item.selected { transform:translateY(-3px); }
  ...
  .progress-bar-track {
    height:6px; border-radius:3px; background:${C.border2}; overflow:hidden;
  }
  .progress-bar-fill {
  ...
  .game-row {
    ...
    padding:10px 16px; border-radius:12px; border:1px solid ${C.border};
    background:${C.card2}; transition:border-color 0.15s;
...
function RankBadge({ rank }) {
  return (
    <span style={{
      background: RANK_COLORS[rank-1] + "22",
      color:      RANK_COLORS[rank-1],
      border:     `1px solid ${RANK_COLORS[rank-1]}55`,
...
function VIPBadge() {
  return (
    <span style={{
      ...
      color:"#111", borderRadius:20, padding:"2px 10px",
      fontSize:11, fontWeight:900, fontFamily:"'DM Sans',sans-serif",
      letterSpacing:0.5,
    }}>⭐ VIP</span>
  );
}

// ─── BADGE SHOWCASE PICKER ────────────────────────────────────────────────────
function BadgePicker({ equipped, onEquip }) {
  const [picking, setPicking] = useState(null); // slot index 0-2
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      {/* Showcase slots */}
      <div>
        <div style={{ fontSize:12, fontWeight:700, color:C.muted, letterSpacing:1,
            ...
            return (
              <div key={i} className="showcase-slot"
                ...
                style={{
                  ...
                  "--glow": rColor + "66",
                  animation: b ? `glow 3s ease-in-out infinite` : "none",
                }}>
                {b
        ...
        {equipped.filter(Boolean).length > 0 && (
          <div style={{ display:"flex", gap:8, marginTop:8, flexWrap:"wrap" }}>
            {equipped.map((b,i) => b && (
              <span key={i} style={{ fontSize:12, color:RARITY_COLORS[b.rarity],
      ...
      {picking !== null && (
        <div style={{
          background:C.card2, border:`1px solid ${C.a4}55`,
          borderRadius:16, padding:16, animation:"popIn 0.25s ease",
        }}>
          <div style={{ fontSize:12, color:C.a4, fontWeight:700, letterSpacing:1,
              ...
              return (
                <button key={b.id}
                  ...
                  <div style={{ textAlign:"left" }}>
                    <div style={{ fontSize:12, fontWeight:700, color:RARITY_COLORS[b.rarity],
                                  fontFamily:"'DM Sans',sans-serif" }}>{b.name}</div>
        ...
        <div style={{ maxWidth:860, margin:"0 auto", position:"relative", zIndex:1 }}>
          ...
          <div style={{ display:"flex", gap:20, alignItems:"flex-start", marginBottom:24, flexWrap:"wrap" }}>
            ...
            <div style={{ position:"relative", flexShrink:0 }}>
              <div style={{
                ...
                background:`linear-gradient(135deg,${C.a5}44,${C.a4}44)`,
            ...
            <div style={{ flex:1, minWidth:200 }}>
              ...
              <div style={{ display:"flex", gap:8, marginBottom:10, flexWrap:"wrap" }}>
                {equipped.map((b,i) => b && (
                  <div key={i} style={{
                    background: RARITY_BG[b.rarity],
                    border:`1px solid ${RARITY_COLORS[b.rarity]}55`,
                    borderRadius:10, padding:"4px 10px",
                    display:"flex", alignItems:"center", gap:5,
                    animation:`badgePop 0.5s cubic-bezier(.17,.67,.35,1.3) ${i*0.1}s both`,
                    boxShadow:`0 0 12px ${RARITY_COLORS[b.rarity]}22`,
                  }}>
                    <span style={{ fontSize:14 }}>{b.icon}</span>
                    <span style={{ fontSize:11, fontWeight:700, color:RARITY_COLORS[b.rarity] }}>
                      {b.name}
                    </span>
                  </div>
                ))}
            ...
            <div style={{
              ...
              boxShadow:`0 0 20px ${C.a2}18`,
            }}>
              <div style={{ fontSize:24, marginBottom:2 }}>💰</div>
              <div style={{ fontFamily:"'Boogaloo',cursive", fontSize:24, color:C.a2 }}>
                {PLAYER.bluffBucks.toLocaleString()}
              </div>
              <div style={{ fontSize:11, color:C.muted, fontWeight:600 }}>Bluff Bucks</div>
            </div>
          </div>
          ...
          <div style={{ marginBottom:0 }}>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
              <span style={{ fontSize:12, fontWeight:700, color:C.muted, letterSpacing:0.5 }}>
          ...
          <div style={{ display:"flex", gap:2, marginTop:20, overflowX:"auto",
                        paddingBottom:0, borderBottom:`1px solid ${C.border}` }}>
            {tabs.map(t => (
              <button key={t.id} className={`tab-btn ${tab===t.id?"active":""}`}
                onClick={() => setTab(t.id)}
                style={{
                  borderBottom: tab===t.id ? `2px solid ${C.a4}` : "2px solid transparent",
                  borderRadius:"10px 10px 0 0", paddingBottom:10,
      ...
      <div style={{ maxWidth:860, margin:"0 auto", padding:"24px 24px 0" }}>
        ...
        {tab === "overview" && (
          <div style={{ display:"flex", flexDirection:"column", gap:24, animation:"fadeUp 0.4s ease" }}>
            {/* Quick stats grid */}
            <div>
              <h3 style={{ fontFamily:"'Boogaloo',cursive", fontSize:20, marginBottom:14, color:C.text }}>
                📊 Career Stats
              </h3>
            ...
            <div style={{ background:C.card2, border:`1px solid ${C.border}`,
              ...
              <h3 style={{ fontFamily:"'Boogaloo',cursive", fontSize:20, marginBottom:16 }}>
                🏅 Badge Showcase
              </h3>
                  ...
                  <div key={i} className="game-row" style={{ animationDelay:`${i*0.06}s` }}>
                    <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                      <div style={{
                        ...
                        background: g.result==="Win" ? C.a3+"22" : C.a1+"22",
                        border:`1.5px solid ${g.result==="Win" ? C.a3+"66" : C.a1+"44"}`,
                        display:"flex", alignItems:"center", justifyContent:"center",
                        fontSize:18,
                      }}>{g.result==="Win" ? "🏆" : "💀"}</div>
                      <div>
                        <div style={{ fontWeight:700, fontSize:14,
                                      color: g.result==="Win" ? C.a3 : C.a1 }}>{g.result}</div>
                        <div style={{ fontSize:11, color:C.muted }}>{g.date}</div>
                      </div>
                    </div>
                    <div style={{ display:"flex", gap:20, flexWrap:"wrap" }}>
                      <div style={{ textAlign:"center" }}>
                        <div style={{ fontFamily:"'Boogaloo',cursive", fontSize:18,
                                      color: g.pts > 0 ? C.a3 : C.a1 }}>
                          {g.pts > 0 ? `+${g.pts}` : g.pts}
                        </div>
                      ...
                      <div style={{ textAlign:"center" }}>
                        <div style={{ fontFamily:"'Boogaloo',cursive", fontSize:18, color:C.a5 }}>
                          {g.bluffs}
                        </div>
                        <div style={{ fontSize:10, color:C.muted }}>bluffs</div>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:16, animation:"fadeUp 0.4s ease" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              ...
              <span style={{ fontSize:12, color:C.muted, fontWeight:600 }}>
                {CATEGORIES.filter(c=>c.rank===6).length} / {CATEGORIES.length} Legend
              </span>
            </div>
            {CATEGORIES.map((cat, i) => {
              const rank      = cat.rank;
              const rankColor = RANK_COLORS[rank-1];
              const nextRank  = RANKS[rank] || "MAX";
              return (
                    ...
                    <div style={{ textAlign:"right" }}>
                      {rank < 6
                        ? <div style={{ fontSize:11, color:C.muted, fontWeight:600 }}>
                            {cat.progress}% → {nextRank}
                          </div>
                        : <div style={{ fontSize:11, color:C.a2, fontWeight:700 }}>👑 MAX RANK</div>
                      }
                    </div>
                  </div>
                  ...
                  <div style={{ display:"flex", gap:4, marginBottom:10, alignItems:"center" }}>
                    {RANKS.map((r,ri) => (
                      <div key={r} style={{ flex:1, height:6, borderRadius:3,
                        background: ri < rank ? RANK_COLORS[ri] : C.border2,
                        boxShadow: ri < rank ? `0 0 6px ${RANK_COLORS[ri]}66` : "none",
                  ...
                  {rank < 6 && (
                    <div className="progress-bar-track">
                      <div className="progress-bar-fill" style={{
                        "--bar-w": `${cat.progress}%`,
                        background:`linear-gradient(90deg,${rankColor},${RANK_COLORS[rank]||rankColor})`,
              ...
              return (
                <div key={rarity}>
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:12 }}>
                    <h4 style={{ fontFamily:"'Boogaloo',cursive", fontSize:18,
                                 color:RARITY_COLORS[rarity], textTransform:"capitalize" }}>
                      {rarity === "legend" ? "👑" : rarity === "epic" ? "✨" : rarity === "rare" ? "💎" : "⚪"}
                      {" "}{rarity}
                    </h4>
                    <span style={{ fontSize:11, color:C.muted, fontWeight:600 }}>
                      {group.filter(b=>b.earned).length}/{group.length} earned
                    </span>
                  </div>
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))", gap:10 }}>
                    {group.map((b,i) => (
                      <div key={b.id} className="badge-item"
                        style={{
                          background: b.earned ? RARITY_BG[b.rarity] : C.card2,
                          border:`2px solid ${b.earned ? RARITY_COLORS[b.rarity]+"55" : C.border}`,
                        ...
                        <div style={{ fontWeight:800, fontSize:13,
                                      color: b.earned ? RARITY_COLORS[b.rarity] : C.muted }}>
                          {b.name}
                        </div>
                        <div style={{ fontSize:11, color:C.muted, marginTop:3,
                                      fontWeight:500, lineHeight:1.4 }}>{b.desc}</div>
                        {!b.earned && (
                          <div style={{ marginTop:6, fontSize:10, color:C.muted2, fontWeight:700 }}>
                            🔒 Locked
                          </div>
                        )}
                      </div>
        ...
        {tab === "achievements" && (
          <div style={{ display:"flex", flexDirection:"column", gap:12, animation:"fadeUp 0.4s ease" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4 }}>
              ...
              <span style={{ fontSize:12, color:C.muted, fontWeight:600 }}>
                {ACHIEVEMENTS.filter(a=>a.done).length} / {ACHIEVEMENTS.length} complete
              </span>
            </div>
            {ACHIEVEMENTS.map((a, i) => {
              ...
              return (
                <div key={i} style={{
                  background:C.card2, border:`1px solid ${a.done ? C.a3+"44" : C.border}`,
                  ...
                  <div style={{
                    ...
                    background: a.done ? C.a3+"22" : C.border,
                    display:"flex", alignItems:"center", justifyContent:"center",
                    fontSize:24, border:`1.5px solid ${a.done ? C.a3+"66" : C.border2}`,
                  }}>{a.done ? a.icon : "🔒"}</div>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ display:"flex", justifyContent:"space-between",
                                  alignItems:"center", marginBottom:4 }}>
                      <span style={{ fontWeight:800, fontSize:14,
                                     color: a.done ? C.text : C.muted }}>{a.name}</span>
                      <span style={{ fontSize:12, fontWeight:700,
                                     color: a.done ? C.a3 : C.muted }}>
                        {a.done ? "✓ Complete" : `${a.progress}/${a.total}`}
                      </span>
                    </div>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4 }}>
              ...
              <div style={{ display:"flex", gap:16, fontSize:13, fontWeight:700 }}>
                <span style={{ color:C.a3 }}>
                  {RECENT_GAMES.filter(g=>g.result==="Win").length}W
                </span>
                <span style={{ color:C.a1 }}>
                  {RECENT_GAMES.filter(g=>g.result==="Loss").length}L
                </span>
              </div>
            </div>
            {RECENT_GAMES.map((g,i) => (
              <div key={i} className="game-row" style={{ animationDelay:`${i*0.07}s` }}>
                <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                  <div style={{
                    ...
                    background: g.result==="Win" ? C.a3+"22" : C.a1+"18",
                    border:`1.5px solid ${g.result==="Win" ? C.a3+"55" : C.a1+"44"}`,
