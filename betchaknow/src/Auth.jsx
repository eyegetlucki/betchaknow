...
function LoginScreen({ onSwitch, onSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setErrors({ pw: err.message || "Login failed. Check your email and password." });
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            <button className="oauth-btn" disabled={!!oauthLoading} onClick={() => handleOAuth("google")}>
              {oauthLoading === "google"
                ? <span style={{ width:18, height:18, border:"2px solid #fff3", borderTopColor:"#fff", borderRadius:"50%", display:"inline-block", animation:"spin 0.7s linear infinite" }} />
                : <GoogleIcon />}
              Continue with Google
            </button>
            <button className="oauth-btn" disabled={!!oauthLoading} onClick={() => handleOAuth("discord")}>
              {oauthLoading === "discord"
                ? <span style={{ width:18, height:18, border:"2px solid #fff3", borderTopColor:"#5865F2", borderRadius:"50%", display:"inline-block", animation:"spin 0.7s linear infinite" }} />
                : <DiscordIcon />}
...
function SignUpScreen({ onSwitch, onSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const data = await api.signup({ username, email, password: pw });
      onSuccess({ dest: "verify", userId: data.userId, email });
    } catch (err) {
      setErrors({ email: err.message || "Signup failed. Try a different email or username." });
      setShake(true); setTimeout(()=>setShake(false),500);
    } finally {
      setLoading(false);
    }
      ...
      <AuthCard style={{ animation: shake ? "shake 0.4s ease" : "popIn 0.4s ease" }}>
        <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            <button className="oauth-btn" style={{ flex:1, fontSize:13 }} disabled={!!oauthLoading}
              onClick={() => handleOAuth("google")}>
              {oauthLoading === "google"
                ? <span style={{ width:16, height:16, border:"2px solid #fff3", borderTopColor:"#fff", borderRadius:"50%", display:"inline-block", animation:"spin 0.7s linear infinite" }} />
                : <GoogleIcon size={16} />}
              Google
            </button>
            <button className="oauth-btn" style={{ flex:1, fontSize:13 }} disabled={!!oauthLoading}
              ...
              {oauthLoading === "discord"
                ? <span style={{ width:16, height:16, border:"2px solid #fff3", borderTopColor:"#5865F2", borderRadius:"50%", display:"inline-block", animation:"spin 0.7s linear infinite" }} />
                : <DiscordIcon size={16} />}
              Discord
            </button>
          </div>
...
// ─────────────────────────────────────────────────────────────────────────────
function VerifyScreen({ userId, email, onSuccess }) {
  ...
  const handleVerify = async () => {
    ...
    setLoading(true);
    try {
      const data = await api.verifyEmail({ userId, code: fullCode });
      saveSession({ token: data.token, refreshToken: data.refreshToken, username: data.profile?.username });
      setVerified(true);
  ...
  const handleResend = async () => {
    ...
    setResending(true);
    try {
      await api.resendVerification({ userId, email });
      setResent(true);
...
export default function AuthFlow({ onAuthenticated }) {
  ...
  const handleSuccess = (result) => {
    const dest = typeof result === "string" ? result : result?.dest;
    if (dest === "verify") {
  ...
  return (
    <div style={{
      ...
      {screen === "verify" && <VerifyScreen userId={pendingUserId} email={pendingEmail} onSuccess={handleSuccess} />}
