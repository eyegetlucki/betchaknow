...
const MY_CLUB = {
  id:"c1", name:"The High Rollers", tag:"HROLL", icon:"💎",
  banner:`linear-gradient(135deg, #c77dff, #4d96ff)`,
  description:"Competitive trivia players who play to win. Active daily, friendly community, no toxic vibes!",
  members:34, maxMembers:50, weeklyXP:48200, rank:7, founded:"Jan 2026",
  myRole:"Captain", level:12, xpToNext:8200, xpForLevel:10000,
};
  ...
  return (
    <div className={`friend-row ${friend.isFav ? "fav" : ""}`}>
      ...
      <div style={{ minWidth:0 }}>
        ...
        <div style={{ fontSize:11, color:C.muted, fontWeight:600, marginTop:2, display:"flex", alignItems:"center", gap:6, flexWrap:"wrap" }}>
          ...
          <span style={{ color: friend.status === "in-game" ? C.a4 : friend.status === "online" ? C.a3 : C.muted }}>
            {friend.status === "in-game" ? `Playing · ${friend.game}` : friend.status === "online" ? "Online" : `Offline · ${friend.lastSeen}`}
          </span>
      ...
      <div style={{ display:"flex", gap:6 }}>
        ...
        <button className="ico-btn" title="Message"      onClick={() => onAction("message", friend)}>💬</button>
        <button className="ico-btn" title="View profile"  onClick={() => onAction("profile", friend)}>👤</button>
      </div>
    </div>
  );
}
        ...
        {req.type === "incoming" ? (
          <>
            <button onClick={() => onAction("accept", req)} style={{
              ...
              fontWeight:800, fontSize:12, cursor:"pointer", boxShadow:`0 2px 10px ${C.a3}33`,
            }}>Accept</button>
...
function FriendsPage() {
  ...
  useEffect(() => {
    ...
    api.friends().then(d => {
      const list = (d.friends || []).map(f => ({
        id:       f.id,
        name:     f.username,
        avatar:   f.avatar_icon || "🎯",
        level:    f.level || 1,
        isVIP:    f.is_vip || false,
        isOnline: false,
        status:   "offline",
        lastSeen: "unknown",
        isFav:    false,
        wins:     f.rivalry?.wins   || 0,
        losses:   f.rivalry?.losses || 0,
        friendshipId: f.friendshipId,
      }));
      setFriends(list);
    }).catch(() => {});

    api.friendsPending().then(d => {
      const inc = (d.incoming || []).map(f => ({
        id: f.id, name: f.requester.username, avatar: f.requester.avatar_icon || "🎯",
        level: f.requester.level || 1, isVIP: f.requester.is_vip || false, type: "incoming",
      }));
      const out = (d.outgoing || []).map(f => ({
        id: f.id, name: f.recipient.username, avatar: f.recipient.avatar_icon || "🎯",
        level: f.recipient.level || 1, isVIP: f.recipient.is_vip || false, type: "outgoing",
      }));
      setPending([...inc, ...out]);
    }).catch(() => {});
  }, []);
  ...
  const handleAction = (action, friend) => {
    ...
    if ((action === "decline" || action === "cancel") && isLoggedIn()) {
      api.friendRespond({ friendshipId: friend.id, action: "decline" })
        .then(() => setPending(prev => prev.filter(p => p.id !== friend.id)))
        .catch(() => {});
    }
    const labels = {
  ...
  const handleSendRequest = () => {
    ...
    if (isLoggedIn()) {
      api.friendRequest({ username: addInput.trim() })
        .then(() => showToast(`Friend request sent to ${addInput}!`))
        .catch(e => showToast(e.message || "Error sending request"));
    } else {
      showToast(`Friend request sent to ${addInput}!`);
    }
    ...
    <div style={{ animation:"fadeUp 0.4s ease" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(160px, 1fr))", gap:10, marginBottom:20 }}>
        {[
          ...
          <div key={i} style={{
            background:C.card2, border:`1px solid ${s.color}33`, borderRadius:14, padding:"12px 16px",
            display:"flex", alignItems:"center", gap:10, animation:`fadeUp 0.4s ease ${i*0.05}s both`,
          }}>
      ...
      <div style={{ background:C.card2, border:`1px solid ${C.border}`, borderRadius:14, padding:14, marginBottom:18, display:"flex", gap:10, flexWrap:"wrap", alignItems:"center" }}>
        {!showAdd ? (
          <button onClick={() => setShowAdd(true)} style={{
            ...
            background:`linear-gradient(135deg, ${C.a4}, ${C.a5})`, color:"#fff", fontWeight:800, fontSize:13, cursor:"pointer",
            fontFamily:"'DM Sans',sans-serif", boxShadow:`0 4px 14px ${C.a4}44`,
            display:"inline-flex", alignItems:"center", gap:6,
          }}>➕ Add Friend</button>
        ) : (
          <>
            <input className="input-row" autoFocus placeholder="Enter username..." value={addInput}
              style={{ flex:1, minWidth:200 }} onChange={e => setAddInput(e.target.value)}
              onKeyDown={e => e.key === "Enter" && handleSendRequest()} />
            <button onClick={handleSendRequest} style={{
              padding:"11px 18px", borderRadius:12, border:"none",
              background:`linear-gradient(135deg, ${C.a3}, ${C.a4})`, color:"#fff", fontWeight:800, fontSize:13, cursor:"pointer",
      ...
      <div style={{ display:"flex", gap:8, marginBottom:16, flexWrap:"wrap" }}>
        {[
          ...
          <button key={f.id} onClick={() => setFilter(f.id)} style={{
            ...
            fontWeight:800, fontSize:12, cursor:"pointer", fontFamily:"'DM Sans',sans-serif",
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
        {filtered.length === 0 ? (
          <div style={{ textAlign:"center", padding:"40px 20px", color:C.muted }}>
            ...
            <div style={{ fontSize:12, marginTop:4 }}>{search ? `No matches for "${search}"` : "Try changing your filter"}</div>
          </div>
        ) : filtered.map(f => (
...
function MyClubPage() {
  ...
  const [club,         setClub]         = useState(MY_CLUB);
  const [clubMembers,  setClubMembers]  = useState(CLUB_MEMBERS);
  const [myRole,       setMyRole]       = useState(MY_CLUB.myRole);
  ...
  useEffect(() => {
    ...
    api.myClub().then(d => {
      ...
      setClub({
        ...
        myRole: d.myRole || "Member", level: c.level || 1,
        xpToNext: c.xp_to_next || 0, xpForLevel: c.xp_for_level || 10000,
      });
      const mapped = (d.members || []).map(m => ({
        id: m.profile?.id || m.user_id,
        name: m.profile?.username || "Player",
        avatar: m.profile?.avatar_icon || "🎯",
        level: m.profile?.level || 1,
        role: m.role ? (m.role.charAt(0).toUpperCase() + m.role.slice(1)) : "Member",
        weekXP: m.weekly_xp || 0,
        isOnline: false,
        isMe: m.profile?.username === myUsername,
      }));
      setClubMembers(mapped);
      return api.clubChat(c.id);
    }).then(chatRes => {
      if (!chatRes) return;
      const mapped = (chatRes.messages || []).map(m => ({
        id: m.id,
        user: m.user?.username || "Player",
        avatar: m.user?.avatar_icon || "🎯",
        msg: m.message,
  ...
  const sendMessage = () => {
    ...
    const optimistic = { id: Date.now(), user: myUsername || "You", avatar: myAvatar, msg, time: "now", isMe: true };
    setMessages(prev => [...prev, optimistic]);
    if (isLoggedIn() && clubId) {
      api.sendChat(clubId, { message: msg }).catch(() => {});
    }
  };

  const xpPct     = Math.round((club.xpToNext / club.xpForLevel) * 100);
    ...
    <div style={{ animation:"fadeUp 0.4s ease" }}>
        ...
        <div style={{ position:"relative", zIndex:1 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:14, marginBottom:14 }}>
            <div style={{ display:"flex", gap:14, alignItems:"center" }}>
              <div style={{
                width:64, height:64, borderRadius:18, background:"#0008",
                display:"flex", alignItems:"center", justifyContent:"center",
                fontSize:32, backdropFilter:"blur(4px)", animation:"float 4s ease-in-out infinite",
              }}>{club.icon}</div>
              <div>
            ...
            <div style={{ textAlign:"right" }}>
              <div style={{ fontSize:10, color:"#fffa", fontWeight:800, letterSpacing:0.8, textTransform:"uppercase" }}>Your Role</div>
              <div style={{ marginTop:4, padding:"4px 12px", borderRadius:8, background:"#fff3", backdropFilter:"blur(4px)", color:"#fff", fontWeight:900, fontSize:13, display:"inline-block" }}>👑 {myRole}</div>
            </div>
          </div>
          <p style={{ fontSize:13, color:"#fffe", fontWeight:500, lineHeight:1.5, maxWidth:600 }}>{club.description}</p>
        </div>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(160px, 1fr))", gap:10, marginBottom:20 }}>
        {[
          ...
          { icon:"🏆", label:"Club Rank", value:`#${club.rank}`,                     color:C.a2, sub:"Global" },
          { icon:"⚡", label:"Week XP",   value:(club.weeklyXP||0).toLocaleString(), color:C.a5, sub:"Earned" },
          { icon:"📊", label:"Level",     value:club.level,                          color:C.a3, sub:`${xpPct}% to ${club.level+1}` },
        ].map((s, i) => (
      ...
      <div style={{ background:C.card2, border:`1px solid ${C.border}`, borderRadius:14, padding:16, marginBottom:20 }}>
        ...
        <div style={{ fontSize:11, color:C.muted, fontWeight:600, marginTop:6 }}>
          {Math.round((1 - (club.xpToNext||0)/(club.xpForLevel||10000)) * 100)}% to Club Level {(club.level||1) + 1}
        </div>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"minmax(0, 1.4fr) minmax(0, 1fr)", gap:18, marginBottom:20 }}>
        <div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:6, maxHeight:480, overflowY:"auto", paddingRight:4 }}>
            {sortedMembers.map((m, i) => <MemberRow key={m.id} member={m} rank={i+1} />)}
          </div>
        </div>
        ...
        <div style={{ background:C.card2, border:`1px solid ${C.border}`, borderRadius:16, overflow:"hidden", display:"flex", flexDirection:"column", maxHeight:520 }}>
          ...
          <div style={{ padding:12, borderTop:`1px solid ${C.border}`, display:"flex", gap:8 }}>
            ...
            <button onClick={sendMessage} style={{
              ...
              background: chatMsg.trim() ? `linear-gradient(135deg, ${C.a4}, ${C.a5})` : C.border,
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(180px, 1fr))", gap:10 }}>
          {CLUB_REWARDS.map((r, i) => (
            <div key={i} style={{ background:C.card2, border:`1px solid ${r.unlocked ? C.a3 + "55" : C.border}`, borderRadius:14, padding:"14px 16px", opacity: r.unlocked ? 1 : 0.55, animation:`fadeUp 0.4s ease ${i*0.06}s both` }}>
              ...
              <div style={{ fontSize:11, color: r.unlocked ? C.a3 : C.muted, fontWeight:800, letterSpacing:0.5, textTransform:"uppercase" }}>
...
function BrowseClubsPage() {
  const [search,  setSearch]  = useState("");
  const [toast,   setToast]   = useState("");
  const [clubs,   setClubs]   = useState(RECOMMENDED_CLUBS);

  useEffect(() => {
    api.clubs().then(d => {
      const list = (d.clubs || []).map(c => ({
        id:         c.id,
        name:       c.name,
        tag:        c.tag,
        icon:       c.icon || "🏆",
        members:    c.member_count?.[0]?.count || 0,
        maxMembers: 50,
        weekXP:     c.weekly_xp || 0,
        rank:       c.rank || 0,
        level:      c.level || 1,
        vibe:       c.vibe || "Friendly",
      }));
      if (list.length > 0) setClubs(list);
    }).catch(() => {});
  }, []);
  ...
  const handleJoin = (club) => {
    if (isLoggedIn()) {
      api.joinClub(club.id)
        .then(() => { setToast(`Joined ${club.name}!`); setTimeout(() => setToast(""), 2000); })
        .catch(e => { setToast(e.message || "Error joining club"); setTimeout(() => setToast(""), 2000); });
    } else {
  ...
  return (
    <div style={{ animation:"fadeUp 0.4s ease" }}>
      {toast && (
        <div style={{
          position:"fixed", top:80, left:"50%", transform:"translateX(-50%)",
          background:C.card, border:`1.5px solid ${C.a3}`, borderRadius:12, padding:"10px 20px",
          color:C.a3, fontWeight:700, fontSize:13, zIndex:100, animation:"popIn 0.3s ease",
          boxShadow:`0 8px 32px ${C.a3}33`,
        }}>✓ {toast}</div>
      )}
      ...
      <h3 style={{ fontFamily:"'Boogaloo',cursive", fontSize:20, marginBottom:12 }}>⭐ Top Clubs to Join</h3>
            ...
            <div style={{ position:"relative", zIndex:1 }}>
              <div style={{ display:"flex", gap:12, alignItems:"center", marginBottom:12 }}>
                ...
                <div style={{ minWidth:0, flex:1 }}>
                  ...
                  <div style={{ fontSize:11, color:C.muted, fontWeight:600, marginTop:2 }}>Lvl {club.level} · #{club.rank} globally</div>
                </div>
              </div>
        ...
        <div style={{ maxWidth:1100, margin:"0 auto", position:"relative", zIndex:1 }}>
          ...
          <div style={{ display:"flex", gap:4, overflowX:"auto", borderBottom:`1px solid ${C.border}` }}>
            {[
              ...
              <button key={t.id} onClick={() => setSection(t.id)} className={`nav-tab ${section === t.id ? "active" : ""}`}>
                {t.label}
              </button>
            ))}
          </div>
